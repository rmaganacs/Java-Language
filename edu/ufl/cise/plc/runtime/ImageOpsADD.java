...
//	
	...
	public static BufferedImage setAllPixels(BufferedImage image, ColorTupleFloat val) {
		ColorTupleFloat c = val;
		for (int x = 0; x < image.getWidth(); x++)
			for (int y = 0; y < image.getHeight(); y++) {
				image.setRGB(x, y,  c.pack());
			}
	 ...
	 * @return
	 */
	public static BufferedImage makeConstantImage(int width, int height, ColorTuple colorComponentVal) {
		BufferedImage image = new BufferedImage(width, height,BufferedImage.TYPE_INT_RGB);
		return setAllPixels(image,colorComponentVal);
	}
	public static BufferedImage makeConstantImage(int width, int height, ColorTupleFloat colorComponentVal) {
		BufferedImage image = new BufferedImage(width, height,BufferedImage.TYPE_INT_RGB);
		return setAllPixels(image,colorComponentVal);
	}
